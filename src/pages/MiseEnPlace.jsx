...
export default function MiseEnPlace() {
  ...
  const [lote, setLote]    = useState([]) // [{ receitaId, qtd }]
  const [sheet, setSheet]  = useState(false)

  const filtradas = useMemo(() =>
    (receitas || []).filter(r => norm(r.nome).includes(norm(busca))),
    [receitas, busca]
  )

  function addReceita(rec) {
    setLote(l => {
      const ex = l.find(x => x.receitaId === rec.id)
      if (ex) return l.map(x => x.receitaId === rec.id ? { ...x, qtd: x.qtd + 1 } : x)
      return [...l, { receitaId: rec.id, nome: rec.nome, qtd: 1 }]
    })
  }
  ...
  const necessarios = useMemo(() => {
    const map = {} // nome_insumo -> { insumoId, nome, unidade, necessario }
  ...
  return (
    <>
        ...
        <div className="topbar-inner">
          ...
          {lote.length > 0 && (
            <button
              ...
              style={{ background: temFalta ? '#f59e0b' : 'var(--teal)', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
      ...
      <div className="page-content">
              ...
              return (
                <div key={item.receitaId} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <button onClick={() => setQtd(item.receitaId, item.qtd - 1)} style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid var(--border)', background: 'var(--card-bg)', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                    <input
        ...
        <div>
          {filtradas.map(rec => {
            const noLote = lote.find(x => x.receitaId === rec.id)
            return (
              <div key={rec.id} className="card" style={{ padding: '10px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}
                ...
                {noLote ? (
                  <span style={{ color: 'var(--teal)', fontWeight: 700, fontSize: 13 }}>×{noLote.qtd}</span>
                ) : (
      ...
      {sheet && (
        <>
          ...
          <div className="sheet">
            ...
            {temFalta && (
              <div style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid #f59e0b', borderRadius: 8, padding: '8px 12px', marginBottom: 12, fontSize: 12, color: '#92400e' }}>
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      {ok && (
                        <div style={{ fontSize: 11, color: 'var(--teal)' }}>✓</div>
                      )}
                    </div>
                  </div>
                )
